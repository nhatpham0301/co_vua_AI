...
Hiện các ad unit ID đã được đưa ra khỏi code Dart và đọc từ file `assets/env/config.env`.

### Keys cần cấu hình

| Key                             | Bắt buộc | Mục đích                                          |
| ------------------------------- | -------- | ------------------------------------------------- |
| `ADMOB_ANDROID_BANNER_ID`       | Có       | Banner ad unit cho Android                        |
| `ADMOB_IOS_BANNER_ID`           | Có       | Banner ad unit cho iOS                            |
| `ADMOB_ANDROID_INTERSTITIAL_ID` | Có       | Interstitial ad unit cho Android                  |
| `ADMOB_IOS_INTERSTITIAL_ID`     | Có       | Interstitial ad unit cho iOS                      |
| `ADMOB_ANDROID_APP_ID`          | Nên có   | App ID Android (tham chiếu để sync native config) |
| `ADMOB_IOS_APP_ID`              | Nên có   | App ID iOS (tham chiếu để sync native config)     |

### File liên quan

- `assets/env/config.env`: file local dùng khi chạy app
- `.env.example`: mẫu key để chia sẻ trong repo
- `lib/logic/ad_service.dart`: đọc ad unit IDs từ env (fallback test IDs nếu thiếu key)
- `lib/main.dart`: load env khi app khởi động

### Lưu ý release

- `.env` đã được ignore trong git.
- Trước khi release, thay ID test trong `.env` bằng production IDs.
- App ID native vẫn cần cập nhật đúng ở:
