...
Danh sách ván đấu gần nhất (public, không cần auth). Dùng cho trang chủ và live feed.

**Query Params**

| Param   | Default | Max | Mô tả                    |
| ------- | ------- | --- | ------------------------ |
| `limit` | 10      | 20  | Số ván trả về (1–20)     |
...
Trả về mảng rỗng `[]` nếu chưa có ván nào.
...
| `moveTimeLimit` | integer | `0`        | Giây/nước (0 = không giới hạn) |
...
| --------------- | ------- | ------- | ----------------------------------------- |
| `timeControl`   | string  | —       | Bắt buộc. Xem danh sách time controls trên |
| `moveTimeLimit` | integer | `0`     | Giây/nước (0 = không giới hạn). Chỉ ghép với người cùng giá trị. |
